...
test("Bash: shows command", () => {
  expect(formatPermissionBody("Bash", {command: "make build 2>&1", timeout: 120_000})).toBe("Bash: make build 2>&1")
})

test("Read: shows file_path", () => {
  expect(formatPermissionBody("Read", {file_path: "/Users/max/Projects/parakeet.cpp/memory/MEMORY.md"})).toBe("Read: /Users/max/Projects/parakeet.cpp/memory/MEMORY.md")
})

test("Write: shows file_path, not content", () => {
  expect(formatPermissionBody("Write", {
...
test("Edit: shows file_path, not old/new strings", () => {
  expect(formatPermissionBody("Edit", {
    replace_all: false,
...
test("real Write PermissionRequest: shows file_path, not content", () => {
  ...
  expect(formatPermissionBody("Write", toolInput)).toBe("Write: /Users/max/Projects/claude-notify/PLAN.md")
})
